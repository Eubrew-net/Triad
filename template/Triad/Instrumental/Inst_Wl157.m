...
% close all; publish(fullfile(pwd,'Inst_Wl157.m'),options_pub);
...
close all; 
[tabla_avg,sl_data,dt_data,rs_data,ap_data ]=report_avg(Cal,'grp','events','outlier_flag',{'sl','dt','','','','',''});      
...
table_avg_157.Properties.RowNames=tabla_avg.events;   
...
input.data=table_avg_157(:,2:end); %(solo los números,quitamos las fechas()
...
t.dates=[datenum(2010,6,1),datenum(2017,6,1),datenum(2018,12,15)];
...
%% DSP's 
close all; 
tabla_dsp=report_dispersion(Cal,'grp','month+events','fpath',fullfile(Cal.path_root,'..','DSP'),'process',0,...
                                           'date_range',datenum(2017,1,1):now);  
   ...
   tabla_dsp_157=display_table(tabla_dsp.data(jn,[2:8 15:end]),tabla_dsp.data_lbl([2:7 14:end]),10,'.4f',tabla_dsp.events(jn))
% Command-Print 
 fprintf('\r\nDSP Analysis\r\n'); 
 jn=~isnan(tabla_dsp.data(:,2));
