...
%Cal.file_save  = fullfile('..','..','triad__157_2018.mat');
Cal.file_save  = fullfile('..','triad_comp2018')
%Cal.file_save  = 'triad_2018';
try
    load(Cal.file_save);
catch
    disp('new data');
...
Cal.Date.day0=datenum(2017,6,1);
...
%% Generic
Cal.n_inst=1;
%load(fullfile('..','..','triad__157_2018.mat'),'summary_old','summary_orig_old','summary','summary_orig')
...
%% DT analysis
 DT_analysis(Cal,ozone_raw0,config);%,'plot_flag',0,'DTv',[26 29],...
            % 'date_select',datenum(Cal.Date.cal_year,1,[1 118]));

%%
 [sl_mov_o{1},sl_median_o{1},sl_out_o{1},R6_o{1}]=sl_report_jday(1,sl,Cal.brw_str,...
                               'outlier_flag',1,'diaj_flag',0,'events_raw',events_raw{1},...
                               'hgflag',1,'fplot',1);
%%   no changes                        
%   [sl_mov_a{1},sl_median_a{1},sl_out_a{1},R6_a{1}]=sl_report_jday(1,sl_cr,Cal.brw_str,...
%                                 'outlier_flag',1,'diaj_flag',0,'events_raw',events_raw{1},...
%                                 'hgflag',1,'fplot',1);
...
%% ---- ND Filter plots
   ...
   legend([h1(end,1),h2(end,1)],{'DT=32','DT=28'},'Location','SouthWest','Orientation','Horizontal');
...
% Filters regression (aplicamos filtros de AOD y O3 stricto)
   ...
   lgl_filt{Cal.n_inst}=langley_filter_lvl1(ozone_lgl{Cal.n_inst},'plots',0,...
                                    'airmass',[1.05 4],'O3_hday',2,'N_hday',N_data,...
   ...
   % Time Series
   figure; grid;
   [m_brw,s_brw,n_brw]=grpstats([nd0(:,1) nd3(:,2)-nd0(:,2)],grp_def(nd0(:,1)),{'mean','sem','std'});
...
airm_rang=[1.15,4.0];
 ...
 l_data_op=array2table(brw_indv{Cal.n_inst}(:,:,1),'VariableNames',{'Diaj','F_AM','F_PM','S_AM','S_PM'},'Rownames',cellstr(datestr(brw_indv{Cal.n_inst}(:,1,1))));                
 l_data_ch=array2table(brw_indv{Cal.n_inst}(:,:,2),'VariableNames',{'Diaj','F_AM','F_PM','S_AM','S_PM'},'Rownames',cellstr(datestr(brw_indv{Cal.n_inst}(:,1,2))));                  
...
lgl_table_157=join(t_days,l_data_op);   

% all days table. 
l_data_op.Properties.VariableNames{1}='Date'; 
t_days_all.Properties.VariableNames{1}='Date'; 
langley_table_157=outerjoin(t_days_all,l_data_op);   
...
save(Cal.file_save,'-APPEND','brw_indv','dbs_indv','st_brw','st_dbs','days_lgl','ozone_lgl_dep');
...
writetable(tl_157,'Langley_2017.xls','Sheet',['Op_t_',Cal.brw_str{Cal.n_inst}],'WriteRownames',true);
