...
clear Validos
%% BEFORE ARENOSILLO
[ref,ratio_ref]=join_summary(Cal,summary_old,reference_brw,analyzed_brewer,5,'date_range',datenum(2013,1,[1,160]));
d=size(ratio_ref)
contador=0
for dd=1:1:d(1,1)
    diff=abs(ratio_ref(dd,2))+abs(ratio_ref(dd,3))+abs(ratio_ref(dd,4));
...
%No tiene sentido la comparativa pq solo hay un Brewer en tenerife. el 183 y 185 están de campaña.

% [ref,ratio_ref]=join_summary(Cal,summary_old,1,1,5,'date_range',datenum(2013,1,[161,171]));
% Cal.analyzed_brewer=[1];
% d=size(ratio_ref)
% contador=0
% for dd=1:1:d(1,1)
...
%% AFTER ARENOSILLO
[ref,ratio_ref]=join_summary(Cal,summary_old,1:3,1:3,5,'date_range',datenum(2013,1,[172,365]));
d=size(ratio_ref)
contador=0
for dd=1:1:d(1,1)
    diff=abs(ratio_ref(dd,2))+abs(ratio_ref(dd,3))+abs(ratio_ref(dd,4));
...
% [ref,ratio_ref]=join_summary(Cal,summary_old,1:2,1:2,5,'date_range',datenum(2013,1,[161,171]));
% % ratio_ref_plots
% Cal.analyzed_brewer=[1];
% d=size(ratio_ref)
% contador=0
