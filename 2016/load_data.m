% options_pub.outputDir=fullfile(pwd,'report');   options_pub.showCode=false;
% publish(fullfile(pwd,'load_data.m'),options_pub);

%%  Brewer setup
clear all;
file_setup='calizo2015_setup';
run(fullfile('..','..',file_setup));     % configuracion por defecto

Cal.Date.day0=datenum(2015,1,1);
Cal.Date.dayend=now;%datenum(2015,1,85);
Date.CALC_DAYS=Cal.Date.day0:Cal.Date.dayend; Cal.Date=Date;

Cal.file_save  = 'triad_comp2015';
...
Cal.n_ref=Cal.n_ref;
 for i=Cal.n_ref
    ...
    if i<=3
        ...
        [ozone,log_,missing_]=read_bdata(i,Cal,'/Users/aredondas/CODE/iberonesia/RBCC_E/2015/bdata');
    end
...
Cal.analyzed_brewer=analyzed_brewer; Cal.sl_c=[1,1,0];

Cal.brw=[157 183 185];
%% before, during and after huelva

%% BEFORE HUELVA
[ref,ratio_ref]=join_summary(Cal,summary_old,reference_brw,analyzed_brewer,5,'date_range',datenum(2015,5,[1,20]));
% ratio_ref_plots
[f_hist,f_ev,f_sc,f_smooth]=ratio_ref_plots(Cal,ratio_ref,'plot_smooth',0);

%%  DURING HUELVA
[ref,ratio_ref]=join_summary(Cal,summary_old,1:2,1:2,5,'date_range',datenum(2015,5,[23,40]));
% ratio_ref_plots
...
%% AFTER HUELVA
[ref,ratio_ref]=join_summary(Cal,summary_old,1:3,1:3,5,'date_range',datenum(2015,6,[6,20]));
% ratio_ref_plots
...
%% BEFORE HUELVA
[ref,ratio_ref_alt]=join_summary(Cal,summary,reference_brw,analyzed_brewer,5,'date_range',datenum(2015,5,[1,20]));
% ratio_ref_plots
...
%%  DURING HUELVA
[ref,ratio_ref_al]=join_summary(Cal,summary,1:2,1:2,5,'date_range',datenum(2015,5,[23,40]));
% ratio_ref_plots
Cal.analyzed_brewer=[1,2];
...
%% AFTER HUELVA
[ref,ratio_ref_aln]=join_summary(Cal,summary,1:3,1:3,5,'date_range',datenum(2015,6,[6,20]));
% ratio_ref_plots
Cal.analyzed_brewer=[1,2,3];
Cal.reference_brewer=[1,2,3];
