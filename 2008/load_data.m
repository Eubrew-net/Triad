...
%%  Brewer setup
clear all;
file_setup='calizo2008_setup';
run(fullfile('..','..',file_setup));     % configuracion por defecto

Cal.Date.day0=datenum(2008,1,1);  Cal.Date.dayend=datenum(2008,12,31); 
Date.CALC_DAYS=Cal.Date.day0:Cal.Date.dayend; Cal.Date=Date;

Cal.file_save='triad_comp2008'; % triad_comp_20122014
...
% 157: 19/Abril (110), 29/Junio (181), 21/Diciembre (356)
idx=cellfun(@(x) find(diaj(unique(fix(x(:,1))))==110 | ...
                      diaj(unique(fix(x(:,1))))==181 | ...
                      diaj(unique(fix(x(:,1))))==356),ozone_ds{1},'UniformOutput',0);
...
% 183: 16/Noviembre (321)
idx=cellfun(@(x) find(diaj(unique(fix(x(:,1))))==321),ozone_ds{2},'UniformOutput',0);
