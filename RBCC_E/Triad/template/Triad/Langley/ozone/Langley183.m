...
% close all; publish(fullfile(pwd,'Langley183.m'),options_pub);
...
% Langley processing
airm_rang=[1.15 3.5];
Fcorr=F_corr;  % from config
%Fx_corr{Cal.n_inst}=; see bellow
N_data=12;
O3_std=2.5;
AOD_file=fullfile('..','..','BSRN','180101_181231_Izana.lev15');
CLOUD_file=fullfile('..','..','BSRN','cloudScreening.txt');
%AOD_file=fullfile('aod','160101_170712_Izana.lev15');
%CLOUD_file=fullfile('BSRN','cloudScreening.txt');
...
%% DT analysis
% DT_analysis(Cal,ozone_raw0,config);
%DT_analysis(Cal,ozone_raw0,config,'plot_flag',0,...
 %'date_select',datenum(Cal.Date.cal_year,1,[90,200]));
%      'DTv',[23 24]
%set(gca,'Ylim',[-1,1]);
%%
ii=2
 [sl_mov_o{ii},sl_median_o{ii},sl_out_o{ii},R6_o{ii}]=sl_report_jday(2,sl_cr,Cal.brw_str,...
                               'outlier_flag',0,'diaj_flag',0,'events_raw',events_raw{ii},...
                               'hgflag',1,'fplot',1,'date_range',datenum(2018,3,10));
%%                           
%  [sl_mov_o{ii},sl_median_o{ii},sl_out_o{ii},R6_o{ii}]=sl_report_jday(2,sl,Cal.brw_str,...
%                                'outlier_flag',1,'diaj_flag',0,'events_raw',events_raw{ii},...
%                                'hgflag',1,'fplot',1);



%% ---- ND Filter :ozone difference
...
%% ---- ND Filter :Langley
   ...
   lgl_filt{Cal.n_inst}=langley_filter_lvl1(ozone_lgl{Cal.n_inst},'plots',0,...
                                    'airmass',[1.15,4.5],'O3_hday',2,'N_hday',N_data,'date_range',datenum(2018,1,[90 365]))%,...
   ...
   % Time Series
   figure; grid;
   [m_brw,s_brw,n_brw]=grpstats([nd0(:,1) nd3(:,2)-nd0(:,2) nd4(:,2)-nd0(:,2)],weeknum(nd0(:,1)),{'mean','sem','std'});
   ...
   h=hline(round(median(lmu(:,2:3))),{'-r','-b'},...
          {sprintf('ND#3 Corr. = %d',round(median(lmu(:,2)))),...
...
%% ---- langley from Indiv. Measurements ----
%Fx_corr{Cal.n_inst}=[0,0,0,0,0,0]; % 
%Cal.file_save  = fullfile('..','triad_comp2018')
...
[ ozone_lgl_dep{Cal.n_inst},days_lgl{Cal.n_inst},days_all{Cal.n_inst}]=langley_filter_lvl1(ozone_lgl{Cal.n_inst},'plots',0,...
                       'date_range',datenum(2018,1,[90 365]),...
...
%Cal.file_save  = fullfile('..','triad_comp2018')
save(Cal.file_save,'-APPEND','brw_indv','dbs_indv','st_brw','st_dbs');
...
tl_183=array2table([m_brw(:,2),s_brw(:,2),m_dbs(:,2),s_dbs(:,2),n_brw(:,2)],...
    'VariableNames',str2name( {'ETC','std','ETC corr. (Dobson)','std (Dobson)','N'}),...
    'RowNames',cellstr(datestr(m_brw(:,1),1)));
tl_183.Date=datetime(datestr(m_brw(:,1),1))
writetable(tl_183,'Langley_2017.xls','Sheet',['Op_t_',Cal.brw_str{Cal.n_inst}],'WriteRownames',true);
...
tl_183=array2table(cat(2,events_cfg_chk.data(8,:)',data),'VariableNames',str2name({'ETC_Alt.','brw','std','dbs','std','N'}),'RowNames',varname(strrep(data_tab_brw.evnts,'"','')));
tl_183.Date=datetime(datestr(event_info.dates))
writetable(tl_183,'Langley_2018.xls','Sheet',['Op_',Cal.brw_str{Cal.n_inst}],'WriteRownames',true);
%%
fprintf('Langley por eventos\r\n');
...
data_tab_dbs=meanperiods(mixed_dbs_indv, event_info);
data=[round(data_tab_brw.m(:,2)) round(data_tab_brw.std(:,2)*10)/10 round(data_tab_dbs.m(:,2)) round(data_tab_dbs.std(:,2)*10)/10 data_tab_dbs.N(:,2)];
tl_183m=array2table(data,'VariableNames',str2name({'brw','std','dbs','std','N'}),'RowNames',varname(strrep(data_tab_brw.evnts,'"','')));
tl_183m.Date=datetime(datestr(event_info.dates))
writetable(tl_183m,'Langley_2018.xls','Sheet',['Op_m',Cal.brw_str{Cal.n_inst}],'WriteRownames',true);
...
tl_183=array2table([m_brw(:,2),s_brw(:,2),m_dbs(:,2),s_dbs(:,2),n_brw(:,2)],...
    ...
    'RowNames',cellstr(datestr(m_brw(:,1),1)));
tl_183.Date=datetime(datestr(m_brw(:,1),1))
writetable(tl_183,'Langley_2017.xls','Sheet',['Al_t_',Cal.brw_str{Cal.n_inst}],'WriteRownames',true);
   
%%
fprintf('Langley por  meses\r\n');
event_info=getevents(Cal,'grp','month'); 
data_tab_brw=meanperiods(mixed_brw_indv, event_info);
data_tab_dbs=meanperiods(mixed_dbs_indv, event_info);
data=[round(data_tab_brw.m(:,2)) round(data_tab_brw.std(:,2)*10)/10 round(data_tab_dbs.m(:,2)) round(data_tab_dbs.std(:,2)*10)/10 data_tab_dbs.N(:,2)];
tl_183m=array2table(data,'VariableNames',str2name({'brw','std','dbs','std','N'}),'RowNames',varname(strrep(data_tab_brw.evnts,'"','')));
tl_183m.Date=datetime(datestr(event_info.dates))
writetable(tl_183m,'Langley_2018.xls','Sheet',['Alt_m',Cal.brw_str{Cal.n_inst}],'WriteRownames',true);
...
data=[round(data_tab_brw.m(:,2)) round(data_tab_brw.std(:,2)*10)/10 round(data_tab_dbs.m(:,2)) round(data_tab_dbs.std(:,2)*10)/10 data_tab_dbs.N(:,2)];
% Command-Print 
displaytable(cat(2,events_cfg_chk.data(8,:)',data),{'ETC Alt.','brw','std','dbs','std','N'},8,{'d','d','.1f','d','.1f','d'},data_tab_brw.evnts);

tl_183e=array2table(cat(2,events_cfg_chk.data(8,:)',data),'VariableNames',str2name({'ETC_Alt.','brw','std','dbs','std','N'}),'RowNames',varname(strrep(data_tab_brw.evnts,'"','')));
tl_183e.Date=datetime(datestr(event_info.dates))
writetable(tl_183e,'Langley_2018.xls','Sheet',['Al_',Cal.brw_str{Cal.n_inst}],'WriteRownames',true);
...
legend({'Brw Op. (DT=23)','Dbs. Op. (DT=23)','Brw Chk. (DT=22)','Dbs. Chk. (DT=22)'},'location','NorthEast','FontSize',7);
...
legend({'BrewerMth: DT 23 ns','BrewerMth: DT 22 ns'},'Location','SouthEast','Orientation','Horizontal');
