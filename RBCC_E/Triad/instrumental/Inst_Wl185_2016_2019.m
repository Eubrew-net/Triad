...
% close all; publish(fullfile(pwd,'Inst_Wl185_2016_2019.m'),options_pub);
...
tableop_185=array2table(c,'VariableNames',str2name(events_cfg_op.legend(2:end)));                
tableop_185.Date=datetime(datestr(events_cfg_op.data(1,ia)));                
tableop_185=tableop_185(:,[end,1:end-1])
%% Configs: Check
...
tablec_185=array2table(c,'VariableNames',str2name(events_cfg_op.legend(2:end)))                
tablec_185.Date=datetime(datestr(events_cfg_op.data(1,ia)));                
tablec_185=tablec_185(:,[end,1:end-1])                
...
%%
table_avg_185=array2table(tabla_avg.data,'VariableNames',str2name(['fecha',tabla_avg.data_lbl]));               
table_avg_185.Date=datetime(datestr(tabla_avg.data(:,1)));   
table_avg_185.Properties.RowNames=varname(tabla_avg.events);   
table_avg_185=table_avg_185(:,[end,1:end-1])      
try
  rows2vars(table_avg_185)
catch
  disp('transpose');
end

input.data=table_avg_185(:,2:end); %(solo los n�meros,quitamos las fechas()
...
writetable(avg_t,'avg_ev_2019_185.txt');
...
%%
%tabla_tc_185=display_table(tabla_tc.data(:,2:end)',tabla_tc.events,12,'.2f',tabla_tc.data_lbl)

%[tabla_tc,sl_raw_185]=report_temperature(Cal,OP_config,ALT_config,'grp','events');
% Command-Print
%  fprintf('\r\nTemp. Dependence\r\n'); 
...
tabla_sc_185=display_table(tabla_sc.data(:,2:end),tabla_sc.data_lbl,15,'.0f',tabla_sc.events)
writetable(tabla_sc_185,'SC_summary_2019_185.txt')
writetable(tabla_sc_185,'SC_summary_2019_185.xls')
...
printfiles_report(ix',Cal.dir_figs,'Width',Width,'Height',Height);
close all
writetable(tabla_sc_185,'SC_summary_185.txt')
writetable(tabla_sc_185,'SC_summary_185.xls')
...
tabla_hl_185=display_table(tabla_HS.data(:,2:end)',tabla_HS.events,12,'.4f',tabla_HS.data_lbl)
tabla_hs_185=display_table(tabla_HL.data(:,2:end)',tabla_HL.events,12,'.4f',tabla_HL.data_lbl)
...
writetable(dsp_week,'dsp_2019_185.txt')


t_dsp_185=display_table(tabla_dsp_week.data(jn,[1,2 15,17]),tabla_dsp_week.data_lbl([1,2 15,17]),10,'.4f',tabla_dsp_week.events(jn));
...
%tabla_dsp_185=display_table(tabla_dsp.data(jn,[2:7 14:end]),tabla_dsp.data_lbl([2:7 14:end]),10,'.4f',tabla_dsp.events(jn))
...
writetable(dsp_ev,'dsp_avg_2019_185.txt')
...
t_dsp_185=display_table(tabla_dsp.data(jn,[1,2 15,17]),tabla_dsp.data_lbl([1,2 15,17]),10,'.4f',tabla_dsp.events(jn))
