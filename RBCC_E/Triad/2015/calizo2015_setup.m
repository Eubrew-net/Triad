...
if exist('alberto','file' )
  ...
  if ispc
     Cal.path_root=fullfile(cell2mat(regexpi(pwd,'^[A-Z]:', 'match')),'CODE','iberonesia');
      ...
      [linuxStat,linuxUser]=system('whoami'); % get the user name to determine its home
      linuxUser=strtrim(linuxUser); % remove any extra spaces added by matlab
      Cal.path_root=fullfile(filesep,'home',linuxUser,'CODE','iberonesia')
  end
end
path(genpath(fullfile(Cal.path_root,'matlab')),path);
Cal.file_save='calizo_2015.mat';
Cal.campaign='Izana 2015 (Canary Islands, Spain)';
