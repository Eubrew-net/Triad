...
%%  Brewer setup
clear all;
file_setup='calizo2014_setup';
run(fullfile('..','..',file_setup));     % configuracion por defecto

Cal.Date.day0=datenum(2009,1,1);  Cal.Date.dayend=datenum(2009,12,31);
Date.CALC_DAYS=Cal.Date.day0:Cal.Date.dayend; Cal.Date=Date;

Cal.file_save  = 'triad_comp2014';
...
Cal.analyzed_brewer=analyzed_brewer; Cal.sl_c=[0,1,1];
...
clear Validos
%% BEFORE ARENOSILLO
[ref,ratio_ref]=join_summary(Cal,summary_old,reference_brw,analyzed_brewer,5,'date_range',datenum(2009,1,[1,249]));
d=size(ratio_ref);
contador=0;
for dd=1:1:d(1,1)
    diff=abs(ratio_ref(dd,2))+abs(ratio_ref(dd,3))+abs(ratio_ref(dd,4));
...
[f_hist,f_ev,f_sc,f_smooth]=ratio_ref_plots(Cal,ratio_ref,'plot_smooth',1);
[f_hist,f_ev,f_sc,f_smooth]=ratio_ref_plots(Cal,Validos,'plot_smooth',1);
fi=fopen('Before_arenosillo_operativa_Validos.txt','w');
for kk=1:1:contador
    fprintf(fi,'%f %f %f %f %f/n',Validos(kk,1),Validos(kk,2),Validos(kk,3),Validos(kk,4),Validos(kk,5));
end
fclose(fi);
fi=fopen('Before_arenosillo_operativa_rechazados.txt','w');
for kk=1:1:(d(1,1)-contador)
    fprintf(fi,'%f %f %f %f %f/n',rechazados(kk,1),rechazados(kk,2),rechazados(kk,3),rechazados(kk,4),rechazados(kk,5));
end
fclose(fi);
...
[ref,ratio_ref]=join_summary(Cal,summary_old,1:2,1:2,5,'date_range',datenum(2009,1,[250,260]));
...
[f_hist,f_ev,f_sc,f_smooth]=ratio_ref_plots(Cal,ratio_ref,'plot_smooth',1);
[f_hist,f_ev,f_sc,f_smooth]=ratio_ref_plots(Cal,Validos,'plot_smooth',1);
fi=fopen('During_arenosillo_operativa_Validos.txt','w');
for kk=1:1:contador
    fprintf(fi,'%f %f %f %f %f/n',Validos(kk,1),Validos(kk,2),Validos(kk,3),Validos(kk,4));
end
fclose(fi);
fi=fopen('During_arenosillo_operativa_rechazados.txt','w');
for kk=1:1:(d(1,1)-contador)
    fprintf(fi,'%f %f %f %f %f/n',rechazados(kk,1),rechazados(kk,2),rechazados(kk,3),rechazados(kk,4));
end
fclose(fi);
%% AFTER ARENOSILLO
[ref,ratio_ref]=join_summary(Cal,summary_old,1:3,1:3,5,'date_range',datenum(2009,1,[261,365]));
d=size(ratio_ref);
contador=0;
for dd=1:1:d(1,1)
    diff=abs(ratio_ref(dd,2))+abs(ratio_ref(dd,3))+abs(ratio_ref(dd,4));
...
[f_hist,f_ev,f_sc,f_smooth]=ratio_ref_plots(Cal,ratio_ref,'plot_smooth',1);
[f_hist,f_ev,f_sc,f_smooth]=ratio_ref_plots(Cal,Validos,'plot_smooth',1);
fi=fopen('After_arenosillo_operativa_Validos.txt','w');
for kk=1:1:contador
    fprintf(fi,'%f %f %f %f %f/n',Validos(kk,1),Validos(kk,2),Validos(kk,3),Validos(kk,4),Validos(kk,5));
end
fclose(fi);
fi=fopen('After_arenosillo_operativa_rechazados.txt','w');
for kk=1:1:(d(1,1)-contador)
    fprintf(fi,'%f %f %f %f %f/n',rechazados(kk,1),rechazados(kk,2),rechazados(kk,3),rechazados(kk,4),rechazados(kk,5));
end
fclose(fi);
...
[f_hist,f_ev,f_sc,f_smooth]=ratio_ref_plots(Cal,ratio_ref,'plot_smooth',1);
[f_hist,f_ev,f_sc,f_smooth]=ratio_ref_plots(Cal,Validos,'plot_smooth',1);
fi=fopen('Before_arenosillo_alternativa_Validos.txt','w');
for kk=1:1:contador
    fprintf(fi,'%f %f %f %f %f/n',Validos(kk,1),Validos(kk,2),Validos(kk,3),Validos(kk,4),Validos(kk,5));
end
fclose(fi);
fi=fopen('Before_arenosillo_alternativa_rechazados.txt','w');
for kk=1:1:(d(1,1)-contador)
    fprintf(fi,'%f %f %f %f %f/n',rechazados(kk,1),rechazados(kk,2),rechazados(kk,3),rechazados(kk,4),rechazados(kk,5));
end
fclose(fi);
...
[f_hist,f_ev,f_sc,f_smooth]=ratio_ref_plots(Cal,ratio_ref,'plot_smooth',1);
[f_hist,f_ev,f_sc,f_smooth]=ratio_ref_plots(Cal,Validos,'plot_smooth',1);
fi=fopen('During_arenosillo_alternativa_Validos.txt','w');
for kk=1:1:contador
    fprintf(fi,'%f %f %f %f %f/n',Validos(kk,1),Validos(kk,2),Validos(kk,3),Validos(kk,4));
end
fclose(fi);
fi=fopen('During_arenosillo_alternativa_rechazados.txt','w');
for kk=1:1:(d(1,1)-contador)
    fprintf(fi,'%f %f %f %f %f/n',rechazados(kk,1),rechazados(kk,2),rechazados(kk,3),rechazados(kk,4));
end
fclose(fi);
