...
% depuramos días malos:
% 157: 04/Octubre (277)
idx=cellfun(@(x) find(diaj(unique(fix(x(:,1))))==277),ozone_ds{1},'UniformOutput',0);
ozone_ds{1}(cellfun(@(x) ~isempty(x),idx))=[]; ozone_raw{1}(cellfun(@(x) ~isempty(x),idx))=[]; 
ozone_raw0{1}(cellfun(@(x) ~isempty(x),idx))=[]; config{1}(cellfun(@(x) ~isempty(x),idx))=[];
sl{1}(cellfun(@(x) ~isempty(x),idx))=[]; sl_cr{1}(cellfun(@(x) ~isempty(x),idx))=[];

% 183: 13/Jan (13), 05/May (125), 11/Jun (162), 04/Sep (247)
idx=cellfun(@(x) find(diaj(unique(fix(x(:,1))))==13  | ...
                      diaj(unique(fix(x(:,1))))==125 | ...
                      diaj(unique(fix(x(:,1))))==162 | ...                      
                      diaj(unique(fix(x(:,1))))==247),ozone_ds{2},'UniformOutput',0);
ozone_ds{2}(cellfun(@(x) ~isempty(x),idx))=[]; ozone_raw{2}(cellfun(@(x) ~isempty(x),idx))=[]; 
ozone_raw0{2}(cellfun(@(x) ~isempty(x),idx))=[]; config{2}(cellfun(@(x) ~isempty(x),idx))=[];
sl{2}(cellfun(@(x) ~isempty(x),idx))=[]; sl_cr{2}(cellfun(@(x) ~isempty(x),idx))=[];
