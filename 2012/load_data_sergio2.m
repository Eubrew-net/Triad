...
Cal.Date.day0=datenum(2012,1,1);  Cal.Date.dayend=datenum(2012,12,31);
Date.CALC_DAYS=Cal.Date.day0:Cal.Date.dayend; Cal.Date=Date;

Cal.file_save  = 'triad_comp2012';
...
for kk=1:1:contador
    fprintf(fi,'%f %f %f %f %f\n',Validos(kk,1),Validos(kk,2),Validos(kk,3),Validos(kk,4),Validos(kk,5))
end
fclose(fi)
fi=fopen('Todo_año_operativa_rechazados.txt','w')
for kk=1:1:(d(1,1)-contador)
    fprintf(fi,'%f %f %f %f %f\n',rechazados(kk,1),rechazados(kk,2),rechazados(kk,3),rechazados(kk,4),rechazados(kk,5))
end
fclose(fi)

clear Validos
%% BEFORE AROSA
[ref,ratio_ref]=join_summary(Cal,summary_old,reference_brw,analyzed_brewer,5,'date_range',datenum(2012,1,[1,196]));
d=size(ratio_ref)
contador=0
for dd=1:1:d(1,1)
    diff=abs(ratio_ref(dd,2))+abs(ratio_ref(dd,3))+abs(ratio_ref(dd,4));
...
[ref,ratio_ref]=join_summary(Cal,summary_old,1:2,1:2,5,'date_range',datenum(2012,1,[197,208]));
...
for kk=1:1:contador
    fprintf(fi,'%f %f %f %f\n',Validos(kk,1),Validos(kk,2),Validos(kk,3),Validos(kk,4))
end
fclose(fi)
fi=fopen('During_arosa_operativa_rechazados.txt','w')
for kk=1:1:(d(1,1)-contador)
    fprintf(fi,'%f %f %f %f\n',rechazados(kk,1),rechazados(kk,2),rechazados(kk,3),rechazados(kk,4))
end
fclose(fi)
clear Validos
%% AFTER AROSA
[ref,ratio_ref]=join_summary(Cal,summary_old,1:3,1:3,5,'date_range',datenum(2012,1,[209,365]));
d=size(ratio_ref)
contador=0
for dd=1:1:d(1,1)
    diff=abs(ratio_ref(dd,2))+abs(ratio_ref(dd,3))+abs(ratio_ref(dd,4));
...
[f_hist,f_ev,f_sc,f_smooth]=ratio_ref_plots(Cal,ratio_ref,'plot_smooth',1);
[f_hist,f_ev,f_sc,f_smooth]=ratio_ref_plots(Cal,Validos,'plot_smooth',1);
fi=fopen('After_arosa_alternativa_validos.txt','w')
for kk=1:1:contador
    fprintf(fi,'%f %f %f %f %f\n',Validos(kk,1),Validos(kk,2),Validos(kk,3),Validos(kk,4),Validos(kk,5))
end
fclose(fi)
fi=fopen('After_arosa_alternativa_rechazados.txt','w')
for kk=1:1:(d(1,1)-contador)
    fprintf(fi,'%f %f %f %f %f\n',rechazados(kk,1),rechazados(kk,2),rechazados(kk,3),rechazados(kk,4),rechazados(kk,5))
end
fclose(fi)
