...
for i=1:3
    ...
    if i==1
        ...
        ap_table{i}=array2table(sortrows(ap{i}),'VariableNames',label_ap');
        ...
        ap_table{i}=array2table(sortrows(ap{i},1),'VariableNames',label_ap');
        data=[ap_ev{i}.m(:,2:end),ap_ev{i}.std(:,2:end)];
        l=size(data,2)/2;
    ...
    plot(apt.Date,apt{:,temp}-round(median(apt{:,temp})))
    clickableLegend(apt.Properties.VariableNames(temp))
    datetick('x',12,'keepticks','keeplimits')
    grid
    vline_v(events{i}.dates,'-',strrep(events{i}.labels,'_',' '))
    ...
    %% Voltajes SL
    f=figure
    temp=strmatch('SL',apt.Properties.VariableNames);
    diff=apt{:,temp}-round(median(apt{:,temp}));
    j=isoutlier(diff,'quartiles','ThresholdFactor',3);
    
    % outliers
    outl=['outliers:';cellstr(datestr(apt.Date(any(j,2))))]
    h=annotation('textbox', 'String', outl);
    ...
    grid
    vline_v(events{i}.dates,'-',strrep(events{i}.labels,'_',' '))
    ylabel('SL lamp (I/V)difference to the median');
    title(num2str(brewer(i)))
    %% HG
    f=figure
    temp=strmatch('Hg',apt.Properties.VariableNames);
    if ~isempty(temp)
        diff=apt{:,temp}-round(median(apt{:,temp}));
        j=isoutlier(diff,'quartiles','ThresholdFactor',3);
        ...
        grid
        vline_v(events{i}.dates,'-',strrep(events{i}.labels,'_',' '))
        ylabel('HG lamp (I/V)difference to the median');
        title(num2str(brewer(i)))
    end
    
    
    %%
    temp=strmatch('HV',apt.Properties.VariableNames);
    f=figure
    j=isoutlier(apt{:,temp},'quartiles','ThresholdFactor',5);
    plot(apt.Date,apt{:,temp},':o')
    %legend(apt.Properties.VariableNames(temp))
    clickableLegend(apt.Properties.VariableNames(temp))
    datetick('x',12,'keepticks','keeplimits')
    grid
